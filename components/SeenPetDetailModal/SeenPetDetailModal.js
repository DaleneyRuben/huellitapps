...
const SeenPetDetailModal = ({ visible, onClose, notification }) => {
  if (!notification) return null;

  const petTypeText = notification.petType === 'cat' ? 'gatito' : 'perrito';

  // Use notification's coordinates if available, otherwise use default
  const seenLocation = {
    latitude: notification.latitude || DEFAULT_MAP_LOCATION.latitude,
    longitude: notification.longitude || DEFAULT_MAP_LOCATION.longitude,
  };

  // Create region to center map on seen location
  const mapRegion = {
      ...
      <ModalOverlay>
        <ModalContent>
          <ScrollView showsVerticalScrollIndicator={false}>
            <ContentCard>
              <Title>Se Vio a {notification.petName} en:</Title>
                  ...
                  <DescriptionContainer>
                    <DescriptionText>
                      {notification.description ||
                        `Se Vio a ${notification.petName.toLowerCase()} en ${notification.location || 'una ubicación'}`}
                    </DescriptionText>
                  </DescriptionContainer>
                </DescriptionSection>
              </InfoRow>
              ...
              <MapSection>
                <MapLabel>
                  Ubicación donde se vio a {notification.petName}:
                </MapLabel>
                <MapContainer>
                  <Map
                    pets={[
                      {
                        id: notification.petId || 'seen',
                        petName: notification.petName,
                        latitude: seenLocation.latitude,
                        longitude: seenLocation.longitude,
                        type: notification.petType || 'cat',
                      },
                    ]}
                    height={250}
                ...
                {notification.location && (
                  <LocationText>{notification.location}</LocationText>
                )}
              </MapSection>
...
export default SeenPetDetailModal;
